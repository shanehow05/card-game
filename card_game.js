...
const ranks = ['ace','2','3','4','5','6','7','8','9','10','jacks','queens','kings']
const deck = []
...
temporaryValue = shuffledDeck[currentIndex] // current index starts at placeholder 51 and will take whatever card is there, this spot will be assigned the temporaryValue and save it as this variable.  

shuffledDeck[currentIndex] = shuffledDeck[randomIndex] // Following, we will take the spot of the current index and replace it with a random index placeholder card 

shuffledDeck[randomIndex] = temporaryValue // After that we will take that random index placeholder and assign it to the place holder of the temporaryValue 
// 
...
// 5. Done.
function greet (){
  const name = getInput('Welcome to my card game, What is your name')
  console.log(name)
  return name 
}
...
let input = getInput("Do you believe the following card will be 'h' or 'l' than your current card?")
if(input == 'h') {
...
return compare (card1,card2) >0
  ...
  console.log("You need to guess 'h' or 'l', You dont get any points for this round.")
  return false 
}
}
...
function playGame () {
  const deck = shuffle(buildDeck()) // how to call a function inside of a function 
  ...
  while (score < 5 && score < deck.length) {
    ...
    if(guess(currentCard,nextCard)){
      score++
      console.log(`Congratualions, This is your score! ${score}`)

    } else {
      console.log('You were incorrect, You get no points')
    }
    currentCard = nextCard
      }
      deck.length === 0 ? console.log("You're out of cards, You lost") : console.log("You won!") 
